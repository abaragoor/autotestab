...
# ── Load story from user_story.txt ─────────────────────────────────────────
STORY_FILE = Path(__file__).parent / "user_story.txt"

def load_story() -> str:
    if not STORY_FILE.exists():
        print(f"[ERROR] user_story.txt not found at {STORY_FILE}")
        print("  → Create a file called user_story.txt in the same folder as pipeline.py")
        print("  → Paste your user story text into it and re-run.")
        sys.exit(1)
    story = STORY_FILE.read_text().strip()
    if not story:
        print("[ERROR] user_story.txt is empty. Please add your user story text.")
        sys.exit(1)
    return story
...
def main():
    ...
    print(f"  Story loaded from: user_story.txt")
    print("=" * 60)
    print(f"\nStory: {story[:120]}...")
    ...
    # Run the pipeline
    test_data    = step1_normalize_and_generate(story)
